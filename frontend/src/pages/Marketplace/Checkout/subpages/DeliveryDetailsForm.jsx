import { Box, Text, FormLabel, FormControl, Input, Divider, Checkbox, Heading, Stack, Select, useToast, Button } from '@chakra-ui/react'
...
const DeliveryDetailsForm = () => {
  ...
  const handleSubmit = async() => {
    if (firstName && lastName && phoneNumber && deliveryAddress && selectedState && selectedCity){
      try {
      ...
      <FormControl>
        <Stack direction={{base: 'column', md: 'column', lg: 'row'}} spacing={7} justifyContent='space-between' mb='7' alignItems='center'>
          <Box w={{base: '100%', lg: '50%'}}>
            ...
            <Input type='text' w='100%' value={firstName} onChange={(e) => setFirstName(e.target.value)} />
          </Box>
          <Box w={{ base: '100%', lg: '50%' }}>
            <FormLabel fontSize={14}>Last Name</FormLabel>
            <Input type='text' w='100%' value={lastName} onChange={(e) => setLastName(e.target.value)} />
          </Box>
        </Stack>
        ...
        <Stack direction={{ base: 'column', md: 'column', lg: 'row' }} spacing={7} justifyContent='space-between' mb='7' alignItems='center'>
          <Box w={{ base: '100%', lg: '50%' }}>
            ...
            <Input type='text' w='100%' value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} />
          </Box>
          <Box w={{ base: '100%', lg: '50%' }}>
            <FormLabel fontSize={14}>Additional Phone Number</FormLabel>
            <Input type='text' w='100%' value={additionalPhoneNumber} onChange={(e) => setAdditionalPhoneNumber(e.target.value)} />
          </Box>
        </Stack>
        ...
        <Stack direction='column' spacing={7} justifyContent='space-between' mb='7' alignItems='center'>
          <Box w='100%'>
            ...
            <Input type='text' w='100%' value={deliveryAddress} onChange={(e) => setDeliveryAddress(e.target.value)} />
          </Box>
          <Box w='100%'>
            <FormLabel fontSize={14}>Additional Information</FormLabel>
            <Input type='text' w='100%' value={additionalInformation} onChange={(e) => setAdditionalInformation(e.target.value)} />
          </Box>
        </Stack>

        <Stack direction={{ base: 'column', md: 'column', lg: 'row' }} spacing={7} justifyContent='space-between' alignItems='center' mb='3'>
          <Box w={{ base: '100%', lg: '50%' }}>
        ...
        <Box p='3' textAlign='right' mt='3'>
          ...
          <Button bg='#00BFB2' color='white' fontSize={15} fontWeight='medium' onClick={handleSubmit}>Save</Button>
        </Box>
      </FormControl>
    </Box>
  )
